	...
	<div class="fullGrid center">
		...
		<div class="wrapperIn2">
			...
			<!--rightSidebar-->
			 	...
			 	<form action="${pageContext.request.contextPath}/showStockExchange" method="POST" onsubmit="checkFrSelct()" >
			
				<div class="order-left">
					<h2 class="pageTitle"><i class="fa fa-bar-chart-o" aria-hidden="true"></i>  Physical Stock Exchange</h2>
			...
			<div class="col-md-2" >
						<div class="col1title">Select Franchisee :-</div>
					</div>
					<div class="col-md-3">
						
						<select name="franchisee_id" class="form-control chosen"
							tabindex="4" id="franchisee_id"    required>

 							<option disabled selected >Select Franchisee</option>
 							<c:forEach items="${frList}" var="franchisee" varStatus="count">
 							<c:choose>
									<c:when test="${franchisee.frId==frId}">
									<option value="${franchisee.frId}" selected="selected" ><c:out value="${franchisee.frName}" /></option>
									</c:when>
									<c:otherwise>
									<option value="${franchisee.frId}" ><c:out value="${franchisee.frName}" /></option>
									</c:otherwise>
							...
							<div class="colOuter" id="sellBillAdd">
								<div class="col4full" align="center">
									<input name="" class="buttonsaveorder"  value="Add Sell Bill"
										id="substk" onclick="franIdSelcted()" type="submit" style="display:${btnDisplayStyle}">
								</div>
							</div>
...
<script>
function franIdSelcted(){
	//alert(document.getElementById("franchisee_id").value);
	var frId=document.getElementById("franchisee_id").value;
